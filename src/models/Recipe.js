...
class Recipe {
  ...
  addKeywordText(text) {
    ...
    const addWordOccurrence = (word) => {
      ...
      return selWord(word).then(rows => {
        ...
        return (rset) ? hasWord(rset.id, decodeId(this.id)) : addWord(word, decodeId(this.id))
      }).then(rows => rows)
    }

    return this.getKeywords(text).reduce((cur, next) => {
  ...
  save(user) {
    ...
    const update = (rcp) => {
      ...
      const qargs = [ this.headline, this.body, this.notes, decodeId(this.id) ]
...
Recipe.prototype.getKeywords = (phrase) => {
  phrase = striptags(phrase).replace(/&\w+;/g, '')
  return _.difference(phrase.match(/\b[\w+]+\b/g), stopwords)
}
